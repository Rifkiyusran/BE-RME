...
public function edit(Request $request, User $user, $id)
{
    ...
    public function delete($pasien)
    {
        try {
            ...
            $pasien->pelayanan_kb()->whereHas('pasien', function ($query) use ($pasien) {
                $query->where('ID_PASIEN', $pasien->ID_PASIEN);
            })->delete();

            // Hapus data imunisasi terkait pasien
            $pasien->pelayanan_imunisasi()->whereHas('pasien', function ($query) use ($pasien) {
                $query->where('ID_PASIEN', $pasien->ID_PASIEN);
            })->delete();

            // Hapus data keluarga terkait pasien
            if ($pasien->keluarga) {
                $pasien->keluarga->delete();
            }
            // Hapus data pasien
            ...
            return response()->json(['message' => 'Data pasien berhasil di hapus'], 200);
        } catch (\Exception $e) {
            return response()->json(['message' => 'Error: ' . $e->getMessage()], 500);
        }
    }
